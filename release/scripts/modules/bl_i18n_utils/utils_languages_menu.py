...
def gen_menu_file(stats, settings):
    ...
    langs_cats = [[] for _ in range(len(limits))]
    highest_uid = 0
    ...
    data_lines = [
        ...
        f"# Highest ID currently in use: {highest_uid}",
    ]
    ...
    for cat, langs_cat in zip(limits, langs_cats):
        ...
        if langs_cat:
            data_lines.append(f"0:{cat[1]}:")
        else:
            # Do not write the category if it has no language!
            data_lines.append(f"# Void category! #0:{cat[1]}:")
        # ...and all matching language entries!
        for uid_num, label, uid, flag in langs_cat:
            if flag == OK:
                data_lines.append(f"{uid_num}:{label}:{uid}")
            else:
                # Non-existing, commented entry!
                data_lines.append(f"# {FLAG_MESSAGES[flag]} #{uid_num}:{label}:{uid}")
