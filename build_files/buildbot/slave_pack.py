...
if builder.find('cmake') == -1:
...
elif 'win' in builder or 'mac' in builder:
    os.chdir(build_dir)
    ...
    blender_full_version = '.'.join(blender_version[:2])
    ...
    if builder.endswith('vc2015'):
        platform += "-vc14"
    builderified_name = f'blender-{blender_full_version}-{git_hash}-{platform}'
        # NOTE: Blender 2.8 is already respected by blender_full_version.
    if branch not in ['', 'blender2.8']:
        builderified_name = f"{branch}-{builderified_name}"

    os.rename(result_file, f"{builderified_name}.zip")
        # create zip file
    try:
        ...
        z.write(f"{builderified_name}.zip")
        z.close()
        sys.exit(retcode)
    ...
    os.system(f'cp -r {icons} {install_dir}')
    os.system(f"chmod 755 {os.path.join(install_dir, 'blender-softwaregl')}")

        # Construct archive name
    package_name = f'blender-{blender_version}-{blender_hash}-linux-{blender_glibc}-{blender_arch}'

        # NOTE: Blender 2.8 is already respected by blender_full_version.
    if branch not in ['', 'blender2.8']:
        package_name = f"{branch}-{package_name}"

    upload_filename = f"{package_name}.tar.bz2"

    print("Creating .tar.bz2 archive")
    upload_filepath = f'{install_dir}.tar.bz2'
