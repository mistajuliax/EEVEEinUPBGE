...
LA_BlenderLauncher::LA_BlenderLauncher(GHOST_ISystem *system, Main *maggie, Scene *scene, GlobalSettings *gs,
									   int argc, char **argv, bContext *context, rcti *camframe, ARegion *ar, int alwaysUseExpandFraming)
	:LA_Launcher(system, maggie, scene, gs, scene->gm.aasamples, argc, argv),
	m_context(context),
	m_ar(ar),
	m_camFrame(camframe),
